...
export class Serializer {
  ...
  static serialize = async(req: Request, res: Response, next: Function) => { 
    ...
    try {
      ...
      if (!res.locals.data) { return next( expectationFailed('Data not found') ) };
  ...
  static deserialize = async (req: Request, res: Response, next: Function) => {
    ...
    if ( process.env.CONTENT_TYPE === APPLICATION_MIME_TYPE['application/json'] ) { return next(); }
    ...
    if ( !req.body.data ) { return next( badData('Malformed payload: data attribute not found') ) };
