...
export class MulterConfiguration {
   ...
   * @param {string} destination As main destination
   */
  storage(destination?: string) {
    return this.multer.diskStorage({
      ...
      filename: function (req: Request, file, next: Function) {
        let name = filenamify( filename(file.originalname), { replacement: '-', maxLength: 123 } )
          .replace(' ', '-')
          .replace('_', '-')
          .toLowerCase()
          .concat('-')
   ...
   * 
   * @param {string?} destination Directory where file will be uploaded
   * @param {number?} filesize Max file size authorized
   * @param {array?} wildcards Array of accepted mime types
   */
  get( options?: IUpload ) {
    ...
    return { 
      ...
      fileFilter: function(req: Request, file, next: Function) { 
        ...
        catch(e) { next( expectationFailed(e.message) ); }
      } 
    };
  }
